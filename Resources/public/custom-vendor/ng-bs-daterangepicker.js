		...
		.directive('input', ['$compile', '$parse', '$filter', function($compile, $parse, $filter) {
			return {
				...
				link: function($scope, $element, $attributes, ngModel) {
					...
					options.minDate = $attributes.minDate && moment($parse($attributes.minDate)($scope));
					options.maxDate = $attributes.maxDate && moment($parse($attributes.maxDate)($scope));
                    ...
                    //Solve initial range values
                    options.startDate = $attributes.ngModel && $parse($attributes.ngModel)($scope).startDate;
                    options.endDate = $attributes.ngModel && $parse($attributes.ngModel)($scope).endDate;
					options.locale = $attributes.locale && $parse($attributes.locale)($scope);
					options.showDropdowns = $attributes.showDropdowns && $parse($attributes.showDropdowns)($scope);
					options.linkedCalendars = $attributes.showDropdowns && $parse($attributes.linkedCalendars)($scope);
					options.alwaysShowCalendars = $attributes.alwaysShowCalendars && $parse($attributes.alwaysShowCalendars)($scope);
