...
paella.dataDelegates.MHAnnotationServiceDefaultDataDelegate = Class.create(paella.DataDelegate,{
	read:function(context,params,onSuccess) {
		...
		paella.ajax.get({url: '/videoeditor/api/annotation/annotations.json', params: {episode: episodeId, type: "paella/"+context}},	
	...
	write:function(context,params,value,onSuccess) {
		...
		paella.ajax.get({url: '/videoeditor/api/annotation/annotations.json', params: {episode: episodeId, type: "paella/"+context}},	
			function(data, contentType, returnCode) { 
				...
				if (annotations.length == 0 ) {
					paella.ajax.put({ url: '/videoeditor/api/annotation/',
						params: {
							episode: episodeId, 
							type: 'paella/' + context,
							value: value,
					...
					paella.ajax.put({ url: '/videoeditor/api/annotation/'+ annotationId, params: { value: value }},	
						function(data, contentType, returnCode) { onSuccess({}, true); },
						function(data, contentType, returnCode) { onSuccess({}, false); }
					);
				}
			...
			function(data, contentType, returnCode) {
 				...
 				if(annotations) {
					...
					for ( var i=0; i< annotations.length; ++i) {
						...
						asyncLoader.addCallback(new paella.JSONCallback({url:'/videoeditor/api/annotation/'+annotationId}, "DELETE"));
					}
