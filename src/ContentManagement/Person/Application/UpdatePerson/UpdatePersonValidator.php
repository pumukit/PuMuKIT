<?php

declare(strict_types=1);

namespace App\ContentManagement\Person\Application\UpdatePerson;

use App\Shared\Domain\Validator\UuidValidator;

final class UpdatePersonValidator
{
    public static function validate(UpdatePersonRequest $request): void
    {
        UuidValidator::validate($request->id, 'Person ID');

        if (empty($request->name)) {
            throw new \InvalidArgumentException('Person name cannot be empty');
        }

        if (null !== $request->email && !filter_var($request->email, FILTER_VALIDATE_EMAIL)) {
            throw new \InvalidArgumentException(
                sprintf('Invalid email format: %s', $request->email)
            );
        }
    }
}
