...
class CPUsController extends AbstractController
{
    ...
    public function __construct(CpuService $cpuService, JobExecutor $jobExecutor)
    {
    ...
    public function maintenanceAction(Request $request): Response
    {
        ...
        if ((true === $activate || 'activate' === $activate || 'true' === $activate || '' === $activate)
            || (null !== $deactivate && (false === $deactivate || 'false' === $deactivate))) {
            $activateMaintenance = 'activate';
        } elseif ((true === $deactivate || 'deactivate' === $deactivate || 'true' === $deactivate || '' === $deactivate)
     ...
     */
    public function switchMaintenanceAction(string $activateMaintenance, string $cpuName): Response
    {
        $cpu = $this->cpuService->getCpuByName($cpuName);
        if (!$cpu) {
        ...
        switch ($activateMaintenance) {
            ...
            case 'deactivate':
                $this->cpuService->deactivateMaintenance($cpuName);
                for ($i = 0; $i < $cpu['max']; ++$i) {
