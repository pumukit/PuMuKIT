...
class OpencastImportService
{
    ...
    public function importRecording($opencastId, $invert = false, User $loggedInUser = null)
    {
        ...
        if ($seriesOpencastId) {
            ...
            if (!$series) {
                ...
                $series = $this->importSeries($seriesTitle, $seriesOpencastId, $loggedInUser);
            }
            ...
            if (!$series) {
                $seriesTitle = $this->getMediaPackageField($mediaPackage, 'seriestitle');
                $series = $this->importSeries($seriesOpencastSpatial, $seriesOpencastSpatial, $loggedInUser, true);
            }
        } else {
            $series = $seriesRepo->findOneBy(array('properties.opencast' => 'default'));
            if (!$series) {
                $series = $this->importSeries('MediaPackages without series', 'default', $loggedInUser);
            }
        }
    ...
    private function importSeries($title, $properties, User $loggedInUser = null, $spatial = false)
    {
