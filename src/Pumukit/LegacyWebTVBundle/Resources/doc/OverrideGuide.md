...
Overriding the PumukitLegacyWebTVBundle allows you to change:
...
class PumukitExampleOrgWebTVBundle extends Bundle
{
  ...
  {
    return 'PumukitLegacyWebTVBundle';
  }
}
```
...
Override the `src/Pumukit/LegacyWebTVBundle/Resources/views/custom.css.twig`
...
{% extends 'PumukitLegacyWebTVBundle::default.css.twig' %}
...
```html
<img src="{{ asset('bundles/pumukitlegacywebtv/images/webtv/logo80px.png') }}" class="img-responsive" style="max-height:100%" alt="{% trans %}Logo{% endtrans %}"/>
```
...
{% embed 'PumukitLegacyWebTVBundle:Layout:baseheader.html.twig' %}
  {% block logo_url %}
     <img src="{{ asset('bundles/pumukitlegacywebtv/images/logo.png') }}" class="img-responsive" alt="{% trans %}Logo{% endtrans %}"/>
  {% endblock %}
{% endembed %}
...
```yml
 parameters:
  ...
  pumukit_web_tv.default_pic: '/bundles/pumukitlegacywebtv/images/no_pic.jpg'
