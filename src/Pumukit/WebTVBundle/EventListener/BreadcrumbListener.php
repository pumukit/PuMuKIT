...
class BreadcrumbListener
{
    ...
    public function __construct(DocumentManager $documentManager, Container $container, Router $router, Session $session, $translator, $homeTitle = 'home', $parentWeb = null)
    {
        ...
        $this->activeBreadcrumb = $this->container->getParameter('pumukit_breadcrumb.active');
    }
    ...
    private function createBreadcrumb($request)
    {
        $this->initBreadcrumb();
        ...
        if (in_array($request->attributes->get('_route'), $this->multimediaObjectRoutes)) {
            ...
            $title = $this->container->getParameter("pumukit_breadcrumb.channel_$channelNumber");
    ...
    private function addLastPath()
    {
        if (strpos($this->session->get('breadcrumb_referer'), 'latestuploads')) {
            ...
            $title = $this->container->getParameter("pumukit_breadcrumb.channel_$sReferer[0]");
