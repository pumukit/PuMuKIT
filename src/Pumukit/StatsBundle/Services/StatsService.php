...
class StatsService
{
    ...
    public function getHoursRecordedGroupedBy($fromDate, $toDate, $limit, $page, $criteria, $sort, $groupBy)
    {
        $viewsLogColl = $this->dm->getDocumentCollection('PumukitSchemaBundle:MultimediaObject');
        ...
        $pipeline[] = array('$group' => array('_id' => '$date',
                                              'seconds' => array('$sum' => '$duration'), ),
        );
        //$pipeline[] = array('$project' => array('hours' => array('$divide' => array('$seconds', 3600))));
        $pipeline[] = array('$sort' => array('_id' => $sort));
        $pipeline[] = array('$skip' => $page * $limit);
        $pipeline[] = array('$limit' => $limit);
    ...
    public function getSeriesRecordedGroupedBy($fromDate, $toDate, $limit, $page, $criteria, $sort, $groupBy)
    {
        $viewsLogColl = $this->dm->getDocumentCollection('PumukitSchemaBundle:Series');

        $matchExtra = array();

        if (!empty($criteria)) {
            $series = $this->getSeriesIdsWithCriteria($criteria);
            $matchExtra['_id'] = array('$in' => $series);
        }
        ...
        $pipeline[] = array('$match' => array_merge($matchExtra, array('public_date' => array('$gte' => $fromMongoDate,
                                                                                        '$lte' => $toMongoDate, ))),
        );
        $mongoProjectDate = $this->getMongoProjectDateArray($groupBy, '$public_date');
        $pipeline[] = array('$project' => array('date' => $mongoProjectDate));
        $pipeline[] = array('$group' => array('_id' => '$date',
                                              'numSeries' => array('$sum' => 1), ),
        );
        $pipeline[] = array('$sort' => array('_id' => $sort));
        $pipeline[] = array('$skip' => $page * $limit);
        $pipeline[] = array('$limit' => $limit);
