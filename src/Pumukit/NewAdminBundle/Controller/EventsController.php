...
class EventsController extends AdminController implements NewAdminControllerInterface
{
     ...
     * @param mixed|null $type
     */
    public function listEventActionBySerie(Request $request, $type = null)
    {
        $id = $this->session->get('admin/series/id');
        $page = ($this->session->get('admin/live/event/page')) ?: ($request->query->get('page') ?: 1);
        $session = $this->session;
        $eventPicDefault = $this->pumukitNewAdminAdvanceLiveEventCreateDefaultPic;
    ...
    public function sessionAction(Request $request, MultimediaObject $multimediaObject)
    {
        ...
        if ('POST' === $request->getMethod()) {
            ...
            return new JsonResponse(
                ['sessions' => $multimediaObject->getEmbeddedEvent()->getEmbeddedEventSession(), 'overlap' => $overlap]
            );
        }
